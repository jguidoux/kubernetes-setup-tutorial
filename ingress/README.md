...
For now, if you want to access to an application deploy on the cluster, we have these solutions:
* Using the `kubectl proxy`
* Using the kubectl `port-forward`
* Using a service of type `NodePort`

All these methods are not really user friendly for the user. It would be more interesting to be able to access a deployed application using these ways for example:
* myApp.clusterUrl
* clusterUrl/myApp
...
Before installing an ingress controller. we will need to add a public cluster Url. This will be the unique entry point to the cluster. We will use `MetalLB` to produce this entry point. I choose MetalLb because it is easy to install.
...
Nginx seems to me the one with the best documentation. And it install everything in a specific namespace. So it is easy to uninstall this ingress-controller if we with.But theres is two Nxing Controller:
...
Different between both can be found [here](https://github.com/nginxinc/kubernetes-ingress/blob/master/docs/nginx-ingress-controllers.md). I choose the Kubernetes version because the the install is easyer.
...
```
To check if the ingress controller pods started, run the following command:
```shell script
...
```
We will need an external IP to access the ingress controller. But, it won't come becaude there is no load balancer
...
1. We will need to find which range of IP address. Only a range of 1 IP address will be e enough in our case.
...
```
Look the Internal `INTERNAL-IP` addresses. In my case, all addresses are in the range `172.42.42.xxx`. You will need to choose a subset of 10 addresses in this range. 
For example a range from `172.42.42.200` to `172.42.42.210`.Now let's modifie the file `loadbalancer/metalLB-configmap.yml` to change the field `address-pools.addresses`. In my case this file look like this:
```yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
We don't have DNS server, so we will use the file /etc/hosts to add a dns name. Add this line to the file `/etc/hosts`.
```shell script
172.42.42.200 kubernetes.example.com kubernetes
```
Now we can access the cluster using the url: `kubernetes.example.com`. But we don't have application and neither route. So let's add with an example.
...
1. Let's start by create the namespaces
```shell script
...
`nginx-scv:80/nginx`. Expected was this: `nginx-scv:80`. We need to remove the path `/nginx`. To do that we will use regex:
```yaml
- path: /nginx(/|$)(.*)
```
...
``` 
And chech that the urls `http://kubernetes.example.com/nginx` and `http://kubernetes.example.com/blue` have the expected behavior.
...
```
And chech that the urls `http://kubernetes.example.com/green` have the expected behavior.
